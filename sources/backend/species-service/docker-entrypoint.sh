...
echo "ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ¢â‚¬Å¾ Starting migration process for Species Service..."

# Esperar a que la base de datos estÃ© lista
echo "â³ Waiting for database to be ready..."
until mysql -h species-db -u root -prootpass --skip-ssl -e "SELECT 1" >/dev/null 2>&1; do
...
mysql -h species-db -u root -prootpass --skip-ssl <<-EOSQL
	CREATE DATABASE IF NOT EXISTS potai_species_shadow;
	GRANT ALL PRIVILEGES ON potai_species_shadow.* TO 'root'@'%';
	FLUSH PRIVILEGES;
EOSQL
