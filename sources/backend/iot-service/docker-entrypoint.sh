...
echo "ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ¢â‚¬Å¾ Starting migration process for IoT Service..."

# Esperar a que la base de datos estÃ© lista
echo "â³ Waiting for database to be ready..."
until mysql -h iot-db -u root -prootpass --skip-ssl -e "SELECT 1" >/dev/null 2>&1; do
...
mysql -h iot-db -u root -prootpass --skip-ssl <<-EOSQL
	CREATE DATABASE IF NOT EXISTS potai_iot_shadow;
	GRANT ALL PRIVILEGES ON potai_iot_shadow.* TO 'root'@'%';
	FLUSH PRIVILEGES;
EOSQL
